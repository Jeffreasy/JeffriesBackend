	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			r.Route("/schedule", func(r chi.Router) {
				...
				r.With(authMw).Delete("/", scheduleH.Clear)
			})
			...
			r.Route("/habits", func(r chi.Router) {
				...
				r.With(authMw).Delete("/{id}/incident", habitH.DeleteIncident)
			...
			r.Route("/laventecare", func(r chi.Router) {
				...
				r.With(authMw).Patch("/time-entries/{id}", lcH.UpdateTimeEntry)
				r.With(authMw).Delete("/time-entries/{id}", lcH.DeleteTimeEntry)
			...
			r.Route("/contacts", func(r chi.Router) {
				...
				r.With(authMw).Post("/{id}/channels", contactH.AddChannel)
				r.With(authMw).Patch("/{id}/channels/{channelID}", contactH.UpdateChannel)
				r.With(authMw).Delete("/{id}/channels/{channelID}", contactH.DeleteChannel)
				r.With(authMw).Post("/{id}/organizations", contactH.AddOrganization)
				r.With(authMw).Patch("/{id}/organizations/{orgID}", contactH.UpdateOrganization)
				r.With(authMw).Delete("/{id}/organizations/{orgID}", contactH.RemoveOrganization)
				r.Get("/{id}/interactions", contactH.ListInteractions)
				r.With(authMw).Post("/{id}/interactions", contactH.AddInteraction)
				r.With(authMw).Delete("/{id}/interactions/{interactionID}", contactH.DeleteInteraction)
